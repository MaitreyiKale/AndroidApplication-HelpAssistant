...
import com.example.project.UserFacilityNotes.UserNotes;
...
public class UserNotesDatabaseHelper extends SQLiteOpenHelper {
    ...
    public int updateNote(UserNotes note) {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        // updating row
        return db.update(UserNotes.TABLE_NAME, values, UserNotes.COLUMN_ID + " = ?",
    ...
    public void deleteNote(String note) {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c=db.rawQuery("select COLUMN_ID from note_db where COLUMN_NOTE='"+note+"'",new String[]{});
        while(c.moveToNext()) {
            id= c.getString(0);
        }
        db.delete("notes", "COLUMN_NOTE ='"+note+ "'",null);
        db.close();
    }
